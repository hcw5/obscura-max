...
mod tests {
    ...
    #[test]
    fn test_html_attribute_lookup_is_case_insensitive() {
        let mut rt = setup_runtime("<html><body><div id='x'></div></body></html>");
        let result = rt
            .evaluate(
                "(function(){ const el = document.getElementById('x'); el.setAttribute('DATA-TEST', 'ok'); return [el.getAttribute('data-test'), el.getAttribute('DATA-TEST'), el.hasAttribute('Data-Test')]; })()",
            )
            .unwrap();
        let arr = result.as_array().unwrap();
        assert_eq!(arr[0], serde_json::json!("ok"));
        assert_eq!(arr[1], serde_json::json!("ok"));
        assert_eq!(arr[2], serde_json::json!(true));
    }

    #[test]
    fn test_svg_attribute_lookup_remains_case_sensitive() {
        let mut rt = setup_runtime("<html><body><svg id='s'></svg></body></html>");
        let result = rt
            .evaluate(
                "(function(){ const svg = document.getElementById('s'); svg.setAttribute('viewBox', '0 0 10 10'); return [svg.getAttribute('viewBox'), svg.getAttribute('viewbox')]; })()",
            )
            .unwrap();
        let arr = result.as_array().unwrap();
        assert_eq!(arr[0], serde_json::json!("0 0 10 10"));
        assert_eq!(arr[1], serde_json::json!(null));
    }

    #[test]
    fn test_namespaced_attribute_apis_use_namespace_and_local_name() {
        let mut rt = setup_runtime("<html><body><svg id='s'></svg></body></html>");
        let result = rt
            .evaluate(
                "(function(){ const svg = document.getElementById('s'); const ns='http://www.w3.org/1999/xlink'; svg.setAttributeNS(ns, 'xlink:href', '#icon'); const before=[svg.getAttributeNS(ns,'href'), svg.getAttribute('href')]; svg.removeAttributeNS(ns,'href'); const after=[svg.getAttributeNS(ns,'href'), svg.getAttribute('href')]; return [before, after]; })()",
            )
            .unwrap();
        let arr = result.as_array().unwrap();
        assert_eq!(arr[0][0], serde_json::json!("#icon"));
        assert_eq!(arr[0][1], serde_json::json!(null));
        assert_eq!(arr[1][0], serde_json::json!(null));
        assert_eq!(arr[1][1], serde_json::json!(null));
    }
}
