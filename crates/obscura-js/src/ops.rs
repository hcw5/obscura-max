...
use html5ever::{Attribute, LocalName, Namespace, Prefix, QualName};
...
fn is_html_element(node: &obscura_dom::Node) -> bool {
    matches!(
        &node.data,
        NodeData::Element { name, .. } if name.ns == html5ever::ns!(html)
    )
}

fn maybe_normalize_html_attr_name(dom: &DomTree, node_id: NodeId, name: &str) -> String {
    let is_html = dom.get_node(node_id).map(is_html_element).unwrap_or(false);
    if is_html {
...
fn parse_ns_and_local(input: &str) -> (Namespace, LocalName) {
    let (ns_raw, local_raw) = input.split_once('\0').unwrap_or((input, ""));
    let ns = if ns_raw.is_empty() {
        Namespace::default()
    } else {
        Namespace::from(ns_raw)
    };
    (ns, LocalName::from(local_raw))
}
...
fn op_dom(
    ...
    match cmd.as_str() {
        ...
        "get_attribute" => {
            ...
            let attr_name = maybe_normalize_html_attr_name(dom, node_id, &arg2);
            let val = dom
                .get_node(node_id)
                .and_then(|n| n.get_attribute(&attr_name).map(|s| s.to_string()));
        ...
        "set_attribute" => {
            ...
            if let Some((name, value)) = arg2.split_once('\0') {
                let normalized_name = maybe_normalize_html_attr_name(dom, node_id, name);
                if normalized_name == "id" {
                    let old_id = dom
                        .get_node(node_id)
                        .and_then(|n| n.get_attribute("id").map(|s| s.to_string()));
                    dom.with_node_mut(node_id, |n| {
                        n.set_attribute(&normalized_name, value.to_string())
                    });
        ...
        "get_attribute_ns" => {
            let nid = arg1.parse::<u32>().unwrap_or(0);
            let node_id = NodeId::new(nid);
            let (namespace, local_name) = parse_ns_and_local(&arg2);
            let val = dom.get_node(node_id).and_then(|n| match &n.data {
                NodeData::Element { attrs, .. } => attrs
                    .iter()
                    .find(|a| a.name.ns == namespace && a.name.local == local_name)
                    .map(|a| a.value.clone()),
                _ => None,
            });
            serde_json::to_string(&val).unwrap_or("null".into())
        ...
        "set_attribute_ns" => {
            ...
            let qualified_name = parts.next().unwrap_or("");
            let value = parts.next().unwrap_or("");
            let namespace = if namespace_raw.is_empty() {
                Namespace::default()
            } else {
                Namespace::from(namespace_raw)
            };
            let (prefix, local) = if let Some((p, l)) = qualified_name.split_once(':') {
                (Some(Prefix::from(p)), LocalName::from(l))
            } else {
                (None, LocalName::from(qualified_name))
            };
            dom.with_node_mut(node_id, |n| {
                if let NodeData::Element { attrs, .. } = &mut n.data {
                    ...
                    {
                        attr.name.prefix = prefix.clone();
                        attr.value = value.to_string();
                    } else {
                        attrs.push(Attribute {
                            name: QualName::new(prefix, namespace, local),
        ...
        "remove_attribute" => {
            ...
            dom.with_node_mut(NodeId::new(nid), |n| {
                if let NodeData::Element { attrs, .. } = &mut n.data {
                    attrs.retain(|a| {
                        a.name.local.as_ref() != attr_name.as_str() || !a.name.ns.is_empty()
                    });
                }
            });
            "true".into()
        ...
        "remove_attribute_ns" => {
            ...
            dom.with_node_mut(node_id, |n| {
                if let NodeData::Element { attrs, .. } = &mut n.data {
                    attrs.retain(|a| !(a.name.ns == namespace && a.name.local == local_name));
                }
            });
            "true".into()
        }
